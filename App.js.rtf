...
\
  const calcularValuation = () => \{\
    ...
    if (fat <= 0 || inv < 0 || cxa < 0 || imo < 0 || !data) \{\
      setMensagem("Por favor, insira valores v\'e1lidos para todos os campos, incluindo a data.");\
      return;\
    \}\
    \
    ...
    const novoHistorico = historico.filter(entry => !entry.data.startsWith(mesAno)); // Remove duplicata do m\'eas\
...
\
  const limparHistorico = () => \{\
    setHistorico([]);\
    setMensagem("Hist\'f3rico apagado com sucesso.");\
  \};\
\
  return (\
    <div className="flex flex-col items-center p-6 bg-blue-900 min-h-screen text-white">\
      <Card className="w-full max-w-md p-6 bg-blue-800 border border-blue-700">\
        <CardContent className="flex flex-col gap-4">\
          ...
          <p className="text-sm text-gray-300 text-center">\
            Insira os valores abaixo para calcular e acompanhar a evolu\'e7\'e3o do valuation do Stots.\
          </p>\
          <Label className="text-white">Data do C\'e1lculo</Label>\
          ...
          <Label className="text-white">Caixa Dispon\'edvel (R$)</Label>\
          ...
          <Button onClick=\{limparHistorico\} className="bg-red-600 text-white font-bold hover:bg-red-500 mt-2">Limpar Hist\'f3rico</Button>\
      ...
      \{historico.length > 0 && (\
        <div className="w-full max-w-md mt-6">\
          <h3 className="text-lg font-bold text-center mb-2 text-white">Hist\'f3rico de Valuation</h3>\
